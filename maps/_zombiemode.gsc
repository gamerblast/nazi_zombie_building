...
main(init_zombie_spawner_name)
{
	...
	init_flags();
	// ROUND 20 START
	if(getdvarint("round_start") == 1)
	{
		level.round_number = 1;
	}
	else if(getdvarint("round_start") == 5)
	{
		level.round_number = 5;
	}
	else if(getdvarint("round_start") == 10)
	{
		level.round_number = 10;
	}
	else if(getdvarint("round_start") == 15)
	{
...
init_standard_zombie_anims()
{
	...
	level.scr_anim["zombie"]["sprint2"] = %ai_zombie_sprint_v4;
	level.scr_anim["zombie"]["sprint3"] = %ai_zombie_walk_v1;
	// level.scr_anim["zombie"]["sprint4"] = %ai_zombie_sprint_v4;
...
round_start()
{
	...
	{
		level.zombie_health = 550;
		level.round_number = 5;
		level.first_round = false;
	}
	else if(getdvarint("round_start") == 10)
	{
		level.zombie_health = 1045; 
		level.round_number = 10;
		level.first_round = false;
	}
	else if(getdvarint("round_start") == 15)
	{
		level.zombie_health = 1683;
		level.round_number = 15;
		level.first_round = false;
	}
	else
	{
		level.zombie_health = 2710;
		level.round_number = 20; 
		level.first_round = false;
	}
...
player_revive_death()
{
	...
	if(!(self hasweapon("zombie_tac45")))
	{
	...
	self.bleedout_bar = self createPrimaryProgressBar();
	self.bleedout_bar updateBar( 0.01, 1 / 7 );
	for(i=7; i>=1; i--)
	{
		wait(1);
	}
	...
	if(self.hasquick2 == 1)
	{
		...
		for(x=10; x>=1; x--)
		{
			self iprintln("hey its me ur brother");
			wait(1);
		}
		VisionSetNaked( "zombie_factory", 1 ); 
	}
	
	if(IsDefined(self.angel_bar))
	{
